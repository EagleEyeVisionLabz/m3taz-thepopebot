...
export function useVoiceInput({ getToken, onTranscript, onPartialTranscript, onError, onVolumeChange }) {
  ...
  const fetchVoiceToken = useCallback(async ({ markUnavailableOnError = true } = {}) => {
    const result = await getToken();
    if (!result.error) {
      tokenRef.current = result.token;
      tokenTimeRef.current = Date.now();
      setVoiceAvailable(true);
    } else if (markUnavailableOnError) {
  ...
  const getReadyToken = useCallback(async () => {
    if (tokenRef.current && Date.now() - tokenTimeRef.current < TOKEN_MAX_AGE) {
      ...
      // Pre-fetch the next one in the background. A transient failure here must
      // not flip voiceAvailable=false (which would hide the mic button until
      // remount) — the next on-demand startRecording can re-fetch a fresh token.
      fetchVoiceToken({ markUnavailableOnError: false }).catch(() => {});
      return { token };
    }
