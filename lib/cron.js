...
function loadCrons() {
  ...
  let crons;
  try {
    crons = JSON.parse(fs.readFileSync(cronFile, 'utf8'));
  } catch (err) {
    console.error(`Invalid JSON in CRONS.json, skipping cron load: ${err.message}`);
    console.log('-----------------\n');
    _cronTasks = [];
    return;
  }
  if (!Array.isArray(crons)) {
    console.error('CRONS.json is not an array, skipping cron load');
    console.log('-----------------\n');
    _cronTasks = [];
    return;
  }
