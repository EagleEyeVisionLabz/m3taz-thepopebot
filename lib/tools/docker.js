...
// Upstream project slug ("owner/name") — identifies thepopebot on both Docker Hub
// (image namespace below) and GitHub (release-notes lookup in lib/cron.js).
// Single source of truth so the version-check/upgrade code and image refs stay in sync.
const UPSTREAM_SLUG = 'stephengpope/thepopebot';
 ...
 * @returns {Promise<{containerId: string, containerName: string}>}
 */
async function runInteractiveContainer({ containerName, repo, branch, codingAgent, featureBranch, workspaceId, injectSecrets, systemPrompt, continueSession = true, scope, userId }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const version = process.env.THEPOPEBOT_VERSION;
  const image = `${UPSTREAM_SLUG}:coding-agent-${agent}-${version}`;
  ...
  if (injectSecrets) {
    ...
    // Create per-container API key for agent-secrets access, scoped to the
    // originating user so it reads global secrets + that user's own, but not
    // other users' per-owner secrets. (audit P2: per-job secret scoping)
    const { createAgentJobApiKey } = await import('../db/api-keys.js');
    const { key: agentJobToken } = createAgentJobApiKey(containerName, userId || null);
 ...
 * @returns {Promise<{containerId: string, containerName: string}>}
 */
async function runHeadlessContainer({ containerName, repo, branch, featureBranch, workspaceId, taskPrompt, mode = 'plan', codingAgent, systemPrompt, continueSession = true, injectSecrets, scope, userId }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const version = process.env.THEPOPEBOT_VERSION;
  const image = `${UPSTREAM_SLUG}:coding-agent-${agent}-${version}`;
...
async function runClusterWorkerContainer({ containerName, image, codingAgent, env = [], binds = [], workingDir }) {
  const version = process.env.THEPOPEBOT_VERSION;
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const resolvedImage = image || `${UPSTREAM_SLUG}:coding-agent-${agent}-${version}`;
...
async function resolveHostPath(containerPath) {
  if (_hostMounts === undefined) {
    // Only memoize on a SUCCESSFUL inspect that returns Mounts. On failure leave
    // _hostMounts undefined so the next call retries rather than caching a broken
    // empty result for the lifetime of the process.
    try {
      const { status, data } = await dockerApi('GET', '/containers/thepopebot-event-handler/json');
      if (status === 200 && data.Mounts) {
        ...
        for (const m of data.Mounts) {
          if (m.Type === 'bind' && m.Destination && m.Source) {
            mounts.push({ dest: m.Destination, source: m.Source });
          }
        }
        // Sort by destination length descending for longest-prefix-first matching
        mounts.sort((a, b) => b.dest.length - a.dest.length);
        _hostMounts = mounts;
      } else {
        console.warn(`[resolveHostPath] event-handler inspect returned status=${status} without Mounts; will retry on next call (path=${containerPath})`);
      }
    } catch (err) {
      console.warn(`[resolveHostPath] event-handler inspect failed; will retry on next call (path=${containerPath}):`, err.message || err);
    }
  }
 ...
 */
async function runAgentJobContainer({ agentJobId, repo, branch, title, description, codingAgent, llmModel, scope, userId }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const version = process.env.THEPOPEBOT_VERSION;
  const image = `${UPSTREAM_SLUG}:coding-agent-${agent}-${version}`;
  ...
  // Create per-container API key for agent-secrets access, scoped to the
  // originating user. (audit P2: per-job secret scoping)
  const { createAgentJobApiKey } = await import('../db/api-keys.js');
  const { key: agentJobToken } = createAgentJobApiKey(containerName, userId || null);
  env.push(`AGENT_JOB_TOKEN=${agentJobToken}`);
