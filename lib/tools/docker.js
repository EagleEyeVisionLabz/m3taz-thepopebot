  ...
  if (_hostMounts === undefined) {
    try {
      ...
      if (status === 200 && data.Mounts) {
        ...
        // Only memoize on a SUCCESSFUL inspect; a failed first call must not
        // permanently cache an empty list (which would yield wrong host paths).
        _hostMounts = mounts;
      } else {
        console.warn('[docker] resolveHostPath: inspect returned no mounts; will retry next call');
      }
    } catch (err) {
      console.warn(`[docker] resolveHostPath inspect failed: ${err?.message || err}`);
    }
  }
