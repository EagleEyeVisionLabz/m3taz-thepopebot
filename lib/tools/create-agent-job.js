 ...
 * @param {string} [options.userId] - Originating user id. Threaded into config + container env;
 *   the PR-merge webhook reads it back to address the result message. When absent, the
 *   resulting message broadcasts to subscribed admins.
...
async function createAgentJob(agentJobDescription, options = {}) {
  ...
  // Label which GitHub step failed so partial-failure states are diagnosable
  // (orphaned tree/commit objects, branch-already-exists, rate limiting, etc.).
  const ghStep = async (step, ...args) => {
    try {
      return await githubApi(...args);
    } catch (err) {
      err.message = `[agent-job ${agentJobId.slice(0, 8)}] GitHub step "${step}" failed: ${err.message}`;
      throw err;
    }
  };

  // 1. Get main branch SHA and its tree SHA
  const mainRef = await ghStep('get-main-ref', `${repo}/git/ref/heads/main`);
  const mainSha = mainRef.object.sha;
  const mainCommit = await ghStep('get-main-commit', `${repo}/git/commits/${mainSha}`);
  ...
  const tree = await ghStep('create-tree', `${repo}/git/trees`, {
    method: 'POST',
    body: JSON.stringify({
  ...
  // The title is derived from untrusted job text; collapse to a single line so
  // request-controlled content can't inject commit trailers/instructions.
  const commitTitle = title.replace(/\s+/g, ' ').trim();
  const commit = await ghStep('create-commit', `${repo}/git/commits`, {
    method: 'POST',
    body: JSON.stringify({
      message: `🤖 Agent Job: ${commitTitle}`,
      tree: tree.sha,
      parents: [mainSha],
    }),
  });
  ...
  await ghStep('create-ref', `${repo}/git/refs`, {
    method: 'POST',
    body: JSON.stringify({
