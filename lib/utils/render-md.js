...
// Hard cap on nested include depth to prevent runaway expansion of deep/fan-out include graphs.
...
function render_md(filePath, options = {}) {
  ...
  if (chain.length >= MAX_INCLUDE_DEPTH) {
    console.log(`[render_md] Maximum include depth (${MAX_INCLUDE_DEPTH}) exceeded; truncating include of ${path.relative(PROJECT_ROOT, path.resolve(filePath))}`);
    return '';
  }
  ...
  const withIncludes = content.replace(INCLUDE_PATTERN, (match, includePath) => {
    ...
    if (includeResolved !== PROJECT_ROOT && !includeResolved.startsWith(PROJECT_ROOT + path.sep)) {
      console.log(`[render_md] Include outside project root ignored: ${includePath.trim()}`);
      return match;
    }
