...
export async function getWorkspaceDiffStats(id, authenticatedUser) {
  ...
  try {
    ...
    let currentBranch;
    try {
      const detected = execFileSync('git', ['-c', 'safe.directory=*', 'rev-parse', '--abbrev-ref', 'HEAD'], opts).trim();
      currentBranch = (detected && detected !== 'HEAD') ? detected : null;
    } catch { /* leave null */ }
    let diffRef;
    if (currentBranch) {
      try {
        execFileSync('git', ['-c', 'safe.directory=*', 'rev-parse', '--verify', `origin/${currentBranch}`], { ...opts, stdio: ['ignore', 'pipe', 'ignore'] });
    ...
    const numstat = execFileSync(
      'git', ['-c', 'safe.directory=*', 'diff', '--numstat', diffRef],
    ...
    for (const line of numstat.split('\n')) {
      ...
      if (add !== '-') {
        ...
        if (!Number.isNaN(n)) insertions += n;
      }
      if (del !== '-') {
        const n = parseInt(del, 10);
        if (!Number.isNaN(n)) deletions += n;
      }
    }
    ...
    for (const file of untracked.split('\n')) {
      if (!file) continue;
      try {
        // execFile (no shell) — `file` is an untracked filename from git and must
        // never be interpolated into a shell string (audit P0: host RCE via grep).
        const count = parseInt(execFileSync('grep', ['-c', '', file], opts).trim(), 10);
        insertions += count;
      } catch {
        insertions += 1;
      }
...
/**
 * Build the public URL for a forwarded port. MUST mirror writeTraefikConfig()
 * in port-forwards.js: same subdomain (via forwardSubdomain) and same domain
 * (SSL_DOMAIN over websecure in prod, *.localhost otherwise) so the URL shown
 * to the user always matches the generated Traefik route.
 * @param {string} id - Workspace ID
 * @param {number} port
 * @param {string} token - unguessable per-forward token
 * @returns {string}
 */
function buildForwardUrl(id, port, token) {
  const subdomain = forwardSubdomain(id, port, token);
  const sslDomain = process.env.SSL_DOMAIN;
  return sslDomain
    ? `https://${subdomain}.${sslDomain}`
...
export async function forwardPort(id, port) {
  ...
  // Reuse the token for an already-forwarded port so its URL stays stable;
  // otherwise mint a fresh unguessable token that gates the public subdomain.
  const existing = getForwards(id).get(portNum);
  const token = existing?.token || generateForwardToken();
  ...
  const url = buildForwardUrl(id, portNum, token);
...
export async function listPortForwards(id) {
  ...
  for (const [port, data] of fwds) {
    ports.push({
      port,
      url: buildForwardUrl(id, port, data.token),
