...
export async function getWorkspaceDiffStats(id, authenticatedUser) {
  ...
  try {
    ...
    for (const file of untracked.split('\n')) {
      if (!file) continue;
      try {
        // Count lines in JS (mirrors `grep -c ''`). Never interpolate the
        // filename into a shell: a malicious untracked name like $(...) would
        // otherwise execute on the host during this poll.
        const content = fs.readFileSync(`${repoPath}/${file}`, 'utf8');
        let count = content.length ? content.split('\n').length : 0;
        if (content.endsWith('\n')) count -= 1;
        insertions += count;
      } catch {
        insertions += 1;
