...
export function getConfig(key) {
  ...
  // NOTE: reading an OAuth-token key is a MUTATING call — getNextOAuthToken()
  // advances LRU state (bumps lastUsedAt) and decrypts the picked token. Callers
  // that only need to check presence should not read these keys via getConfig.
  if (key === 'CLAUDE_CODE_OAUTH_TOKEN') {
    return getOAuthTokenCount('claudeCode') > 0 ? getNextOAuthToken('claudeCode') : undefined;
  }
  if (key === 'CODEX_OAUTH_TOKEN') {
    return getOAuthTokenCount('codex') > 0 ? getNextOAuthToken('codex') : undefined;
  }
  ...
  // Try DB (secret or plain config). Distinguish 'absent' (null) from a
  // legitimately-stored falsy value ('', 0, false) — only treat null/undefined
  // as not-set so the env/DEFAULTS fallback is not triggered for real values.
  else if (SECRET_KEYS.has(key)) {
    const v = getConfigSecret(key);
