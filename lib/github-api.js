...
export async function setGitHubVariable(name, value) {
  ...
  try {
    ...
    try {
      ...
      // Only fall through to POST (create new) when the variable doesn't exist (404).
      // Other failures (401/403/422/rate limit/network) must surface as-is.
      if (!/GitHub API 404:/.test(patchErr.message)) throw patchErr;
