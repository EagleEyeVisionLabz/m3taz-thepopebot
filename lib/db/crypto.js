...
export function decrypt(encryptedJson) {
  ...
  try {
    ...
    throw new Error('decrypt: stored value is not a valid envelope (malformed JSON)');
  }
  const { iv, ciphertext, tag, salt: saltB64 } = parsed ?? {};
  if (typeof iv !== 'string' || typeof ciphertext !== 'string' || typeof tag !== 'string') {
    throw new Error('decrypt: envelope missing required iv/ciphertext/tag fields');
  }
