  ...
  if (!req.auth) {
    ...
    if (staleSessionCookies.length > 0) {
      for (const name of staleSessionCookies) {
        // Mirror the attributes Auth.js set the cookie with so the browser accepts
        // the deletion. Browsers reject any Set-Cookie for a `__Secure-`-prefixed
        // name that lacks the `Secure` attribute, which would leave the stale cookie
        // in place and the redirect loop unbroken over HTTPS (production).
