  ...
  if (!req.auth) {
    ...
    if (staleSessionCookies.length > 0) {
      for (const name of staleSessionCookies) {
        // __Secure-/__Host- prefixed cookies are only overwritten if the clearing
        // cookie also carries Secure. Without it the clear is ignored over HTTPS
        // and the undecryptable cookie loops the user forever.
        const isSecurePrefixed = name.startsWith('__Secure-') || name.startsWith('__Host-');
        response.cookies.set(name, '', {
          maxAge: 0,
          path: '/',
          ...(isSecurePrefixed ? { secure: true, sameSite: 'lax', httpOnly: true } : {}),
        });
      }
    }
