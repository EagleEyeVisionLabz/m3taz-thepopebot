...
// Cap the size of an unparsed (non-JSON) fallback line before it is emitted as
// text and persisted. The agent CLI stdout is an untrusted boundary, so a
// misbehaving or compromised agent could otherwise flood the chat DB with an
// arbitrarily large single line.
const FALLBACK_LINE_MAX = 4096;
...
export function mapLine(line, mapper = mapClaudeCodeLine) {
  ...
  try {
    ...
    // Non-JSON lines (NO_CHANGES, PUSH_SUCCESS, AGENT_FAILED, etc.). Bound the
    // size of untrusted agent stdout before it is persisted verbatim.
    const bounded = line.length > FALLBACK_LINE_MAX
      ? `${line.slice(0, FALLBACK_LINE_MAX)}… [truncated ${line.length - FALLBACK_LINE_MAX} chars]`
      : line;
