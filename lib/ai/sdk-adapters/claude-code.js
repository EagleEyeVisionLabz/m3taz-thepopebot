...
export async function* claudeCodeStream({ prompt, workspaceDir, systemPrompt, sessionId, permissionMode, attachments, workspaceId, chatMode, userId }) {
  ...
  // Permission mode — fail closed: only an explicit 'code' mode grants write/execute
  // (matching the container path in lib/ai/index.js). Any unexpected/blank value
  // stays read-only ('plan') instead of escalating to full write access.
