...
export async function* claudeCodeStream({ prompt, workspaceDir, systemPrompt, sessionId, permissionMode, attachments, workspaceId, chatMode, userId }) {
  ...
  if (chatMode === 'agent') {
    ...
    const { key: agentJobToken } = createAgentJobApiKey(`claude-code-sdk-${shortId}`, userId || null);
  ...
  // Permission mode: only the explicit 'code' value grants full write/execute.
  // Any other value (blank, unexpected, traversal-style input) fails closed to
  // read-only 'plan' rather than escalating to bypassPermissions.
  options.permissionMode = permissionMode === 'code' ? 'bypassPermissions' : 'plan';
  ...
  if (attachments?.length) {
    ...
    for (const att of attachments) {
      if (att.category === 'image') {
        ...
        // PDFs are classified as 'image' upstream but must be sent as a
        // 'document' block — an image block with a PDF media type is invalid.
