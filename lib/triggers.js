...
/**
 * Single-quote a value so untrusted request data cannot break out of a shell
 * command. Wraps in single quotes and escapes any embedded single quote.
 * @param {string} value
 * @returns {string}
 */
 ...
 * @param {Object} [opts] - { shellSafe } when true, substituted values are
 *   shell-quoted (use for `command` actions, whose template is run via a shell).
 * @returns {string}
 */
function resolveTemplate(template, context, { shellSafe = false } = {}) {
  return template.replace(/\{\{(\w+)(?:\.(\w+))?\}\}/g, (match, source, field) => {
    ...
    else if (data[field] !== undefined) value = String(data[field]);
    else return match;
...
async function executeActions(trigger, context) {
  for (const action of trigger.actions) {
    try {
      ...
      // Command actions run through a shell, so untrusted substituted values
      // must be shell-quoted. Agent `job` prompts are not shell-executed.
      if (resolved.command) resolved.command = resolveTemplate(resolved.command, context, { shellSafe: true });
