...
export async function POST(request) {
  ...
  // Validate client-supplied scope before forwarding it into the agent's
  // working-directory / skills resolution. Reject absolute paths and any
  // '..' traversal so the scope cannot escape the agents/ subtree.
  if (scope != null && scope !== '') {
  ...
  const stream = createUIMessageStream({
    ...
    execute: async ({ writer }) => {
      ...
      for await (const chunk of chunks) {
        ...
        } else if (chunk.type === 'auto-run') {
          // Server-launched workspace command — surfaces a spinner client-side.
          writer.write({
            type: 'data-auto-run',
            id: `auto-run-${uuidv4().slice(0, 8)}`,
            data: { containerName: chunk.containerName, command: chunk.command },
          });
...
export async function getSidebarCounts(request) {
  ...
  const { getUnreadCountForUser } = await import('../db/messages.js');
  const messages = getUnreadCountForUser(session.user.id);
  let pullRequests = 0;
  try {
    const { getOpenPullRequests } = await import('../tools/github.js');
    pullRequests = (await getOpenPullRequests()).length;
  } catch {}
  return Response.json({ messages, pullRequests });
}
...
export async function getChatDataHandler(request, { params }) {
  ...
  if (chat.userId !== session.user.id) return Response.json(null);
