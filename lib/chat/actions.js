...
// ConvertKit email-updates signup form. Kept server-side so the form ID is not
// baked into the client bundle. ConvertKit's public subscription endpoint does
// not return a useful status to a cross-origin browser POST, so we proxy it from
// the server where the response can actually be inspected.
const CONVERTKIT_FORM_ID = process.env.CONVERTKIT_FORM_ID || '9126548';

/**
 * Subscribe an email address to the product-updates list (ConvertKit).
 * Returns { success: true } only on a confirmed 2xx/3xx from the provider.
 * @param {string} email
 */
export async function subscribeEmailUpdates(email) {
  await requireAuth();
  ...
  try {
    const response = await fetch(
      `https://app.convertkit.com/forms/${CONVERTKIT_FORM_ID}/subscriptions`,
      {
        ...
        body: 'email_address=' + encodeURIComponent(trimmed),
        redirect: 'manual',
      }
    );
    // A successful ConvertKit submit responds 2xx or a 3xx redirect.
    if (response.ok || (response.status >= 300 && response.status < 400) || response.status === 0) {
      return { success: true };
    }
    return { error: 'Signup failed. Please try again.' };
  } catch (err) {
    console.error('Failed to subscribe email updates:', err);
...
export async function initiateOAuthFlow({ secretName, clientId, clientSecret, tokenUrl, scopes, secretType, returnPath }) {
  await requireAdmin();
  if (!secretName || !clientId || !tokenUrl) {
    return { error: 'Missing required fields' };
  }
  try {
    const { createOAuthState } = await import('../oauth/helper.js');

    // The browser no longer receives the stored clientSecret (see
    // getOAuthSecretCredentials). On a re-authorize the caller may omit it, in
    // which case we reuse the secret already stored server-side for this name.
    ...
    if (!effectiveClientSecret) {
      ...
      const stored = getAgentJobSecretOAuthCredentials(secretName);
      if (stored?.clientSecret) effectiveClientSecret = stored.clientSecret;
    }
    if (!effectiveClientSecret) {
    ...
    const state = createOAuthState({ secretName, clientId, clientSecret: effectiveClientSecret, tokenUrl, secretType: secretType || 'agent_job_secret', returnPath: returnPath || '/admin/event-handler/agent-secrets' });
...
/**
 * Get stored OAuth metadata for an agent job secret (for re-authorization pre-fill).
 * Never returns the stored clientSecret to the browser — only the non-sensitive
 * clientId and tokenUrl, plus a flag indicating a secret is on file. The server
 * reuses the stored clientSecret during re-authorization (see initiateOAuthFlow).
 * @param {string} name
 */
export async function getOAuthSecretCredentials(name) {
  ...
  try {
    const { getAgentJobSecretOAuthCredentials } = await import('../db/config.js');
    const creds = getAgentJobSecretOAuthCredentials(name);
    if (!creds) return { error: 'Not an OAuth secret' };
    return {
