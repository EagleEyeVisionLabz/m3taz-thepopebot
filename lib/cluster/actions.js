...
export async function getSessionLog(clusterId, roleShortId, sessionName, file) {
  ...
  // Validate caller-supplied path segments to prevent traversal
  if (!/^[a-f0-9]{8}$/.test(roleShortId)) return null;
  if (!/^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_[a-f0-9]{8}$/.test(sessionName)) return null;

  const { clusterDir } = await import('./execute.js');
  const baseDir = path.join(clusterDir(cluster), 'logs');
  const filePath = path.resolve(baseDir, `role-${roleShortId}`, sessionName, filename);
