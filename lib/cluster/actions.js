...
export async function getSessionLog(clusterId, roleShortId, sessionName, file) {
  ...
  // Reject path-traversal in caller-supplied segments before joining.
  if (!/^[a-f0-9]{8}$/i.test(roleShortId || '')) return null;
  if (!/^[\w.\-]+$/.test(sessionName || '') || sessionName.includes('..')) return null;

  const { clusterDir } = await import('./execute.js');
  const logsBase = path.join(clusterDir(cluster), 'logs');
  const filePath = path.join(logsBase, `role-${roleShortId}`, sessionName, filename);

  // Defense in depth: ensure the resolved path stays inside the cluster logs dir.
  if (!path.resolve(filePath).startsWith(path.resolve(logsBase) + path.sep)) return null;
