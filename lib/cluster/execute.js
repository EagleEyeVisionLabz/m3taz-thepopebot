...
import { getRoleWithCluster, roleShortId } from '../db/clusters.js';
...
export async function acquireAndRunRole(roleIdOrData, payload, trigger) {
  ...
  try {
    ...
    release();
    // If no later trigger chained onto this gate, the chain is idle — drop the
    // entry so _locks doesn't accumulate one entry per distinct role forever.
...
export async function runClusterRole(roleData, payload, trigger) {
  ...
  console.log(`[cluster] Launching role ${roleData.roleName} (${containerName})${payload ? ` payloadBytes=${JSON.stringify(payload).length}` : ''}`);
...
function scheduleWorkerDirCleanup(containerName, dirPath) {
  const POLL_MS = 5000;
  const MAX_POLLS = 360; // ~30 min overall lifetime cap so the interval can't leak forever
  const MAX_CONSECUTIVE_ERRORS = 5; // give up polling after sustained query failures
  const CONFIRM_ABSENT = 2; // require consecutive "absent" reads before deleting (avoid transient empty results)
  let polls = 0;
  ...
  const interval = setInterval(async () => {
    ...
    if (polls > MAX_POLLS) {
      ...
      console.error(`[cluster] Gave up cleanup polling for ${containerName} after ${MAX_POLLS} polls; worker dir not removed: ${dirPath}`);
      return;
    }

    let containers;
    try {
      containers = await listContainers(containerName);
    } catch (err) {
      consecutiveErrors += 1;
      absentStreak = 0;
      if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
        clearInterval(interval);
        console.error(`[cluster] Cleanup polling for ${containerName} aborted after ${consecutiveErrors} consecutive errors; worker dir not removed: ${dirPath}`, err.message);
      }
    ...
    // Require the container to read as absent across consecutive successful polls
    // so a single transient empty result can't delete a still-running worker dir.
    absentStreak += 1;
    if (absentStreak < CONFIRM_ABSENT) return;
